...
namespace periscope
{
    ...
    public:
        span_node(const date_tick& _start, const date_tick& _end)
            : m_start_tick(_start), m_end_tick(_end) {}
        span_node(const date_tick& _start, const date_duration& _dur)
            : m_start_tick(_start), m_end_tick(_start + _dur) {}
        span_node(const span_node& _last, const date_duration& _dur)
            : m_start_tick(_last.m_end_tick), m_end_tick(_last.m_end_tick + _dur) {}
        span_node(const span_node& _last, const date_tick& _end)
            : m_start_tick(_last.m_end_tick), m_end_tick(_end)
        {
            if (_end.operator < (_last.m_end_tick))
                throw std::invalid_argument(format_printer::print("[0]", prompt_id::k_discontinuous_dateline, _last.m_end_tick, _end));
        }
    ...
    {
        template <>
        std::string print<node_shape>(const node_shape& _value)
        {
            switch (_value)
            {
