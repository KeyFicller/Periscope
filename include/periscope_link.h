...
namespace periscope
{
    ...
    {
    public:
        link_base(const std::vector<t>& _nodes) : m_nodes(_nodes) {}
        ~link_base() override = default;

        // TODO: change to friend.
    public:
        void set_handle_manager(handle_manager<t>* _manager)
        {
            link_base<t>::m_handle_manager = _manager;
        }

        handle_manager<t>* handle_manager() const
        {
            return link_base<t>::m_handle_manager;
        }
    ...
    public:
        const t& from() const
        {
            return link_base<t>::m_nodes.at(0);
        }
        const t& to() const
        {
            return link_base<t>::m_nodes.at(1);
        }
    ...
    public:
        ...
        {
            return binary_link<t>::from();
        }
        const t& descendant() const
        {
            return binary_link<t>::to();
        }
    ...
    {
        template <>
        std::string print<link_shape>(const link_shape& _value)
        {
            switch (_value)
            {
